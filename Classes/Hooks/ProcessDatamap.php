...
class ProcessDatamap
{
    ...
    public function processDatamap_afterDatabaseOperations($status, $table, $uid, $fieldArray, $pObj): void
    {
        $this->pObj = $pObj;
        $this->table = $table;
        $this->status = $status;
        $this->fieldArray = $fieldArray;
        $this->uid = (MathUtility::canBeInterpretedAsInteger($uid) ? $uid : $this->pObj->substNEWwithIDs[$uid]);
        ...
        if ($this->table == 'tx_jvevents_domain_model_event') { //do only things when we are in the  event table
            ...
            /** @var Event $event */
            $this->event = $this->eventRepository->findByUidAllpages(intval($this->uid), FALSE);
