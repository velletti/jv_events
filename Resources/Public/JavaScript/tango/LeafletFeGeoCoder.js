function AddressLeaflet() {
    ...
    LeafFE.init = function (element) {
        ...
        if (LeafFE.$debug) {
            console.log("basic variable initialisation frpm div#map data values");
        }
        ...
        if (!$('#t3js-location-map-wrap').length) {
            if (LeafFE.$debug) {
                console.log("div#map has no sub html, so call addMapMarkup()");
            }
            LeafFE.addMapMarkup();
        }
    };
    ...
    LeafFE.addMapMarkup = function () {
        if (LeafFE.$debug) {
            console.log("add markup to div#map ");
        }
        $('div#map').append(
    ...
    LeafFE.createMap = function () {
        ...
        if (((!LeafFE.$latitude || !LeafFE.$longitude) || (LeafFE.$latitude == 0 && LeafFE.$longitude == 0)) && LeafFE.$geoCodeUrl != null) {
            if (LeafFE.$debug) {
                console.log("try to get lat / long from address ");
            }
            LeafFE.geocode();
        }
        ...
        if (LeafFE.$latitude == null || LeafFE.$longitude == null) {
            if (LeafFE.$debug) {
                console.log("Use fallback Lat/log values ");
            }
        ...
        if (LeafFE.$debug) {
            console.log("Try to assign the map  ");
        }
    ...
    LeafFE.geocode = function () {
        ...
        if (LeafFE.$debug) {
            console.log("$geoCodeUrl: " + LeafFE.$geoCodeUrl);
        }
        $.ajax({
            type: 'GET',
    ...
    LeafFE.updateAddress = function () {
        if (LeafFE.$debug) {
            console.log("UpdateAddress function ");
        }
...
document.addEventListener("DOMContentLoaded", function () {
    if( $('#map').length ) {
        ...
        AddressMapInstance.init($('div#map'));
        AddressMapInstance.createMap();
    }
});
